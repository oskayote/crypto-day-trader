                ...
                <div key={i} style={{ background: "#111827", border: "1px solid " + (isWin ? "#22c55e20" : "#ef444420"), borderRadius: 12, padding: mob ? "12px" : "14px 16px" }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: mob ? "1fr 1fr" : "repeat(4,1fr)", gap: mob ? 6 : 8, marginBottom: 8 }}>
                    <div style={{ background: "#0a0e17", borderRadius: 8, padding: "8px 10px" }}>
                      ...
                      <div style={{ fontSize: 13, fontWeight: 600, color: "#f8fafc" }}>${t.investAmt.toLocaleString()}</div>
                    </div>
                    <div style={{ background: "#0a0e17", borderRadius: 8, padding: "8px 10px" }}>
                      <div style={{ fontSize: 9, color: "#64748b" }}>ENTRY → EXIT</div>
                      <div style={{ fontSize: 13, fontWeight: 600, color: "#f8fafc" }}>${fmt(t.entryPrice)} → ${fmt(t.exitPrice)}</div>
                    </div>
                    <div style={{ background: "#0a0e17", borderRadius: 8, padding: "8px 10px" }}>
                      <div style={{ fontSize: 9, color: "#64748b" }}>FEES</div>
                      <div style={{ fontSize: 13, fontWeight: 600, color: "#f97316" }}>${fmt(t.entryFee + (t.exitFee || 0))}</div>
                    </div>
                    <div style={{ background: "#0a0e17", borderRadius: 8, padding: "8px 10px" }}>
                      <div style={{ fontSize: 9, color: "#64748b" }}>COINS</div>
                      <div style={{ fontSize: 13, fontWeight: 600, color: "#94a3b8" }}>{fmt(t.coins)}</div>
                    </div>
                  </div>
